...
def _brief_system_prompt(surface: str) -> str:
    ...
    return (
        ...
        f"reader conclude.\n"
        f"  3. **Pipeline state is not market signal.** The age of an "
        f"attestation IN OUR DATA reflects how recently our pipeline "
        f"extracted the issuer's published report, NOT the issuer's "
        f"audit discipline or the token's risk profile. Fiat issuers "
        f"publish on roughly monthly cadence; the attestation we have "
        f"on file may be 30 to 90 days old simply because the next "
        f"discovery sweep has not landed yet. NEVER frame attestation "
        f"age in our store as 'verification gap', 'market vulnerability', "
        f"'exposure', or 'verification risk'. That language implies the "
        f"issuer failed to publish; usually they did, we haven't picked "
        f"it up yet. Drift between on-chain supply and the attested "
        f"figure IS a market signal (real movement against the audited "
        f"baseline) and can be described as such.\n"
        f"  4. **Stamp the brief with the attestation as-of date.** "
        f"Where the brief leans on attestation-derived figures, say so "
        f"explicitly: 'per the [Month YYYY] attestation' or 'as of "
        f"[YYYY-MM-DD]'. The reader should never need to wonder whose "
        f"data and from when.\n"
        f"  5. The HEADLINE is a single sentence, the bottom line. "
        ...
        f"  6. KEY_POINTS are 2-4 short bullets (~12 words each), the "
        f"figures that matter most for THIS surface, with their "
        f"provenance compressed (e.g. 'Coverage 100.1% per Apr 30 "
        f"attestation' or 'Supply $50B across 6 chains, 4 cross-RPC "
        f"corroborated'). Each bullet referencing an attestation figure "
        f"should name the attestation date. For non-fiat-backed tokens, "
        f"at least one bullet should describe the on-chain redemption / "
        f"collateral mechanism with a link to the protocol's dashboard.\n"
        f"  7. RELEVANT_NEWS_INDICES: from the news_candidates list, "
        ...
        f"  8. Return strict JSON matching the schema.\n"
        f"  9. Headlines and bullets should be plainly readable to "
        f"non-engineers: financial-product copy quality.\n"
        f" 10. VOICE: never use em dashes (—). Use commas, periods, or "
...
def _market_system_prompt() -> str:
    return (
        ...
        "(concentration, drift, fresh news that matters across many "
        "issuers). Do NOT restate counts like '25 tokens tracked' as "
        "the headline. Do NOT frame the age of an extracted "
        "attestation as a 'verification gap' or a market "
        "vulnerability. Issuers publish monthly; attestation age in "
        "our data reflects pipeline lag, not the issuer's discipline.\n"
        ...
        "each panel slice. Lead with the most material MOVEMENT or "
        "ANOMALY visible in the data, not a recap of state. Surface "
        "what is unusual, what concentrates, what's shifted, what is "
        "the largest single mover. Name concrete numbers and tokens.\n"
        "       - concentration: what's striking about the issuer mix "
        "         (single-issuer dominance, top-3 share, the gap "
        "         between #1 and #2).\n"
        "       - backing: what the model mix implies for where market "
        "         risk actually sits today.\n"
        "       - verification: name the gap in PRODUCT terms. The "
        "         categories you'll see are 'attested' (issuer report "
        "         extracted and cited), 'source-only' (issuer page is "
        "         JS-rendered so we cite the page itself via an AI "
        "         summary), and 'on-chain' (no CPA report needed by "
        "         design). NEVER use the words 'stale' or 'blocked'. "
        "         Frame the source-only category as a substitution, "
        "         not a failure.\n"
        "       - drift: which token has moved most against its last "
        "         attestation, in which direction, over how many days. "
        "         BE PRECISE: drift is the market signal (on-chain "
        "         supply moved vs the audited figure). The age of the "
        "         attestation is a SEPARATE thing, do NOT call it a "
        "         'verification gap' or imply the issuer hasn't "
        "         reported. Issuers publish monthly; if our extracted "
        "         attestation is 60+ days old that's our pipeline "
        "         lag, not a market problem. Frame it as 'on-chain "
        "         supply has moved X% since the most recent extracted "
        "         attestation' or similar. If only one token has a "
        "         drift reading, name it as the only currently "
        "         visible movement.\n"
        "       - chains: name the single largest concentration "
        "         (chain, share, what falls under it).\n"
        "       - signals: the one most material recent corpus event "
        "         and what it changes for the market view. Empty "
        "         string if nothing material.\n"
        "     Each insight is 1-2 sentences MAX. NEVER use internal "
        "     vocabulary (canary, polling, fetcher, JS-rendered fetch, "
        "     'AGENTS.md'). Tone: a sharp analyst writing for peers, "
        "     not a dashboard caption, not an operator status report.\n"
        "  7. Return strict JSON matching the schema.\n"
        "  8. Headlines, bullets, and insights should read at "
        "financial-product copy quality.\n"
